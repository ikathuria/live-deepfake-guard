...
export default function App() {
  ...
  const startAnalysis = useCallback(async () => {
    ...
    const processNextChunk = async () => {
      ...
      // 1. Slice Buffer
      const wavBlob = audioBufferToWav(audioBuffer, cursor, CHUNK_DURATION);
      const base64 = await blobToBase64(wavBlob);

      // 2. Call Gemini
      const result = await analyzeAudioSegment(base64, cursor);
      
      // 3. Update State
      setResults(prev => {
        const newResults = [...prev, result];
        // Calculate moving average for risk
      ...
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-800 pb-6">
        ...
        <div className="flex items-center gap-4">
            ...
            <div className="text-right">
                ...
                <div className="text-indigo-400 font-mono text-sm">Gemini 2.5 Flash</div>
            </div>
        </div>
      </header>
